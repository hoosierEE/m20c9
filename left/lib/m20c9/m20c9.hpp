...
typedef const array<array<uint8_t,cols>,rows> MechLayer; /* ASCII */
 ...
 *             col0      col1 */
    for(uint8_t row=0,N=mp.size();row<N;++row){
        ...
        for(uint8_t col=0,M=mp[row].size();col<M;++col) mp[row][col]=digitalRead(tnsy_col_pins[col]);
        digitalWrite(tnsy_col_pins[row],HIGH);
    }
}

void read_caps_raw(CapsRaw&cr){
    for(const auto&touch_pin:cr) cr[touch_pin]=touchRead(tnsy_cap_pins[touch_pin]);
}
